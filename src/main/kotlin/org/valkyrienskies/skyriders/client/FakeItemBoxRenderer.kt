...
class FakeItemBoxRenderer(context: EntityRendererProvider.Context) : EntityRenderer<FakeItemBoxEntity>(context) {
    override fun render(
        entity: FakeItemBoxEntity,
        entityYaw: Float,
        partialTick: Float,
        poseStack: PoseStack,
        ...
        val boxBuffer = bufferSource.getBuffer(RenderType.entityTranslucent(TEXTURE))
        val time = entity.tickCount.toFloat() + partialTick

        poseStack.pushPose()
        poseStack.translate(0.0, 0.42, 0.0)
        poseStack.mulPose(Axis.YP.rotationDegrees(entityYaw + time * 4.8f))
        poseStack.mulPose(Axis.XP.rotationDegrees(time * 2.5f))
    ...
    override fun getTextureLocation(entity: FakeItemBoxEntity): ResourceLocation = TEXTURE
